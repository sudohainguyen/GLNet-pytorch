...
from utils.loss import SoftCrossEntropyLoss2d, FocalLoss
            ...
            for i_batch, sample_batched in enumerate(tbar):
                ...
                if test:
                    ...
                    for i in range(len(images)):
                        if mode == 1:
                            pred = transforms.functional.to_pil_image(classToRGB(predictions_global[i]) * 255.0)
                            pred.save("./prediction/" + sample_batched["id"][i] + "_mask.png")
                        else:
                            pred = transforms.functional.to_pil_image(classToRGB(predictions[i]) * 255.0)
                            pred.save("./prediction/" + sample_batched["id"][i] + "_mask.png")
